...
def lyric(artist, song):
    client = suds.client.Client('http://api.chartlyrics.com/apiv1.asmx?WSDL')
    ...
    response = client.service.SearchLyric(artist, song)
    ...
    for result in results:
        if result.find('master:LyricId', namespaces) is not None and result.find('master:LyricChecksum',
            ...
            response = client.service.GetLyric(lyric_id, lyric_checksum)
